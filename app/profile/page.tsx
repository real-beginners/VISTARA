  ...
  return (
    <AppShell>
      <div className="mx-auto max-w-4xl">
        <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="eyebrow text-xs font-bold text-coral">Your account</p><h1 className="mt-3 font-display text-4xl tracking-[-0.04em] sm:text-5xl">Traveler profile</h1><p className="mt-3 text-sm leading-6 text-muted">A few details that help make every journey feel more like yours.</p></div><Button href="/settings" variant="outline"><Icon name="settings" size={16} /> Settings</Button></div>
        <div className="mt-9 space-y-5">
          ...
          <Card padding="none"><div className="flex items-center gap-2 border-b border-line px-5 pt-5 sm:px-6"><button type="button" className="border-b-2 border-pine px-1 pb-3 text-xs font-bold text-pine">Trips</button><button type="button" className="px-1 pb-3 text-xs font-bold text-muted">Photos</button><button type="button" className="px-1 pb-3 text-xs font-bold text-muted">Saved</button></div><div className="px-5 py-7 text-sm text-muted sm:px-6">Your trips, shared memories, and saved places will gather here.</div></Card>
